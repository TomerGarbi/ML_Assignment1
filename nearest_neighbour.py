...
class KNN_Classifier:
    ...
    def most_common_label(self, labels):
        ...
        for i in range(len(labels)):
            if labels[i] in labels_seen:
                labels_seen[labels[i]] = labels_seen[labels[i]] + 1
            else:
                labels_seen[labels[i]] = 1

        max_label = max(labels_seen, key=labels_seen.get)
        return max_label

    def predict(self, x):
        distances = np.linalg.norm(self.x_train - x, axis=1)
        sorted_indices = np.argsort(distances)
        closest_labels = [self.y_train[sorted_indices[i]] for i in range(self.k)]
        return self.most_common_label(closest_labels)
...
def predictknn(classifier, x_test: np.array):
    ...
    """
    return np.array(classifier.predictions(x_test))
